...
import org.eclipse.bpmn2.modeler.runtime.automatik.features.JbpmTaskFeatureContainer.JbpmAddTaskFeature;
...
import org.eclipse.bpmn2.modeler.runtime.automatik.wid.editor.ParameterDefinition;
import org.eclipse.bpmn2.modeler.runtime.automatik.wid.editor.Work;
import org.eclipse.bpmn2.modeler.runtime.automatik.wid.editor.WorkItemEditor;
...
public class JbpmCustomTaskFeatureContainer extends CustomShapeFeatureContainer {
	...
	protected class JbpmCreateCustomTaskFeature extends CreateTaskFeature {
		...
		public Task createBusinessObject(ICreateContext context) {
			...
			AutomatikRuntimeExtension rx = (AutomatikRuntimeExtension)customTaskDescriptor.getRuntime().getRuntimeExtension();
	...
	private static WorkItemEditor getWorkItemEditor(Task task) {
    	...
    	AutomatikRuntimeExtension rte = (AutomatikRuntimeExtension)rt.getRuntimeExtension();
    	WorkItemDefinition wid = ((AutomatikRuntimeExtension)rte).getWorkItemDefinition(customTaskId);
	...
	public class ConfigureWorkItemFeature implements ICustomFeature {
		...
		public boolean canExecute(IContext context) {
			...
			if (rte instanceof AutomatikRuntimeExtension && context instanceof ICustomContext) {
				PictogramElement[] pes = ((ICustomContext) context).getPictogramElements();
				if (pes.length==1) {
					...
					if (o instanceof Task) {
						...
						for (EStructuralFeature f : features) {
							if ("taskName".equals(f.getName())) { //$NON-NLS-1$
								...
								WorkItemDefinition wid = ((AutomatikRuntimeExtension)rte).getWorkItemDefinition(taskName);
		...
		public void execute(IContext context) {
			...
			IBpmn2RuntimeExtension rte = editor.getTargetRuntime().getRuntimeExtension();
			WorkItemDefinition workItemDefinition = ((AutomatikRuntimeExtension)rte).getWorkItemDefinition(taskName);
