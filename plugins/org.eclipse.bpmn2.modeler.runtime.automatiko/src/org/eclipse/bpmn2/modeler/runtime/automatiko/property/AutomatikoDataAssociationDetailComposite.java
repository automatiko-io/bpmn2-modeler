 ...
 *
 */
public class AutomatikoDataAssociationDetailComposite extends DataAssociationDetailComposite {

	/**
	 * @param section
	 */
	public AutomatikoDataAssociationDetailComposite(AbstractBpmn2PropertySection section) {
		super(section);
	}
	 ...
	 * @param style
	 */
	public AutomatikoDataAssociationDetailComposite(Composite parent, int style) {
		super(parent, style);
	}
	    ...
	    return new DataInputOutputDetailComposite(parent, style) {
	    	...
	    	private String nameValidator(ItemAwareElement object, String name) {
				if (!SyntaxCheckerUtils.isJavaIdentifier(name)) {
					return Messages.AutomatikoDataAssociationDetailComposite_Name_Invalid;
				}
				if (AutomatikoIoParametersDetailComposite.isCustomTaskIOParameter(object, name)) {
					Activity activity = AutomatikoIoParametersDetailComposite.findActivity(object);
					String propertyTabLabel = ModelUtil.getLabel(activity);
					return NLS.bind(Messages.AutomatikoDataAssociationDetailComposite_Name_Reserved,
	...
	public void createBindings(EObject be) {
		...
		if (adapter!=null) {
			...
			if (AutomatikoIoParametersDetailComposite.isCustomTaskIOParameter((ItemAwareElement)be)) {
...
//		}
//		else if (CustomTaskDescriptor.getDescriptor(activity) != null && AutomatikoIoParametersDetailComposite.isCustomTaskIOParameter((ItemAwareElement)be)) {
//			enabled = false;
//		}
//
